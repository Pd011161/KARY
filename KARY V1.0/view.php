...
<body>
  <center>
  <div class="container" style="width: 800px; display:flex; flex-direction:row; justify-content: space-between;  gap: 50px; margin:30px; margin-top: 10px; padding: 10px; 
    background: #ff22bb; 
  ...
  <div class="finit" style="text-align: center; width: 850px;  margin: 10px; align-item: center; padding: 5px; padding-bottom: 20px;
      ...
      <?php
          $track = $_GET['track'];
          $spl = "SELECT * FROM signature WHERE track = '$track' ORDER BY Time ASC Limit 1,1 ";
          $result=$conn->query($spl);
          if(mysqli_num_rows($result)>0){
              $i=1;
              while($row=mysqli_fetch_array($result)){
      ?>
      <img src="<?=$row['signature']?>" style="margin: 10px 5px; width:90%;">
      <h3 style="color: white;"><?=$row['Time']?></h3>
      <?php
        }  
      }
      ?>
  ...
  <div class="tb" style="margin-top: 20px; ">
      ...
      font-size: 20px;">
          <tr>
            ...
            <th scope="col"><h3 style="width: 130px">date-time</h3></th>
            <th scope="col"><h3>city</h3></th>
            <th scope="col"><h3>map</h3></th>
            <th scope="col"><h3 style="width: 180px">status</h3></th>
          </tr>
        </thead>
      ...
      border: 1px solid rgba(255,255,255,0.1);">
          ...
          border: 1px solid rgba(255,255,255,0.1);">
              ...
              <td><h3 style="margin:10px;"><?=$rg['city']?></h3></td> 
              ...
              <td><h3 style="margin:10px; width: 130px"><?=$rg['status']?></h3></td>       
          </tr>
          <?php
            }  
          }
          ...
          <?php
          $track = $_GET['track'];
          // $spl =  "SELECT * FROM kary_scan_receiver WHERE track = '$track' ";
          ...
          border: 1px solid rgba(255,255,255,0.1);">
              ...
              <td><h3 style="margin:10px; width: 100px; color: white;"><?=$row['city']?></h3></td>
              <td style="width: 450px; height: 250px;"> 
                <iframe src="https://www.google.com/maps?q=<?=$row['latitude']?>,<?=$row['longitude']?>&h1=es;z=14&output=embed" 
                  style=" width: 95%; height: 100%; margin:10px;">
                </iframe> 
              </td>
              <td><h3 style="margin:10px;"><?=$row['status']?></h3></td>                           
          </tr>
          <?php
            }  
          }
  ...
  <div class="finit" style="text-align: center; width: 850px;  margin: 10px; align-item: center; padding: 5px; padding-bottom: 20px;
      ...
      0 0 30px #1eff45,
      0 0 60px #1eff45;">
      <h2 style=" color: red; backgroung:red; margin-top: 10px; margin-bottom: -5px">send signature</h2>
     
       <?php
          $track = $_GET['track'];
          $spl = "SELECT * FROM signature WHERE track = '$track' ORDER BY Time ASC Limit 0,1";
          $result=$conn->query($spl);
          if(mysqli_num_rows($result)>0){
              $i=1;
              while($row=mysqli_fetch_array($result)){
